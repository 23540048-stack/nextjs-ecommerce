...
function ShopContent() {
  ...
  // FETCH DATA VÀ CHUẨN HÓA DỮ LIỆU
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [catRes, prodRes, bannerRes] = await Promise.all([
          ...
          api.get(productsEndpoint).catch(() => {
        ...
        // Tạo Map tra cứu Category nhanh theo cả ID lẫn Slug
        ...
        const currentActiveBanner =
          bannerList.find((b) => b.isActive !== false) || bannerList[0] || null;
        setActiveBanner(currentActiveBanner);

        // 3. CHUẨN HÓA DANH SÁCH SẢN PHẨM
        ...
        const formattedList: Product[] = prodList.map((item: any) => {
          ...
          // Trường hợp 1: item.category là Object có sẵn
          if (typeof item.category === "object" && item.category !== null) {
            catId = item.category._id || "";
            catSlug = item.category.slug || "";
            catName = item.category.name || "";
          }
          // Trường hợp 2: item.category chỉ là String ID -> Tra cứu trong categoryMap
  ...
  const filteredProducts = useMemo(() => {
    return rawProducts
      .filter((product) => {
        ...
        // CATEGORY (Lọc linh hoạt theo Slug, ID hoặc SubCategory)
        if (selectedCategory && selectedCategory !== "ALL") {
          const target = selectedCategory.toLowerCase();

          // Tìm category đối ứng trong danh mục để lấy thêm ID/Slug
        ...
        if (selectedPrice === "UNDER_20" && price >= 20) return false;
        if (selectedPrice === "20_60" && (price < 20 || price > 60))
          return false;
        if (selectedPrice === "60_100" && (price < 60 || price > 100))
          return false;
        if (selectedPrice === "ABOVE_100" && price <= 100) return false;
          ...
          <div>
            ...
            <div className="space-y-2 text-xs">
              ...
              {categories.map((cat) => {
                ...
                // SO SÁNH THÔNG MINH CHO CẢ SLUG VÀ ID
                const isSelected =
